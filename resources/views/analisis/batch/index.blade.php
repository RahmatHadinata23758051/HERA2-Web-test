        ...
        <div class="bg-white rounded-xl border border-surface-container-high shadow-sm hover:shadow-md hover:border-primary/30 transition-all duration-300 flex flex-col justify-between overflow-hidden group">
            <div class="p-5 space-y-4">
                <div class="flex justify-between items-start gap-3">
                    ...
                    <form action="{{ route('analisis.batch.destroy', $batch->id) }}" method="POST"
                          onsubmit="return confirm('Apakah Anda yakin ingin menghapus batch &quot;{{ $batch->name }}&quot;? Seluruh data responden di dalamnya akan terhapus permanen.');">
                        @csrf @method('DELETE')
                        <button type="submit" class="p-1.5 text-outline hover:text-error hover:bg-error-container rounded-lg transition-colors" title="Hapus Batch">
