...
class InfluxSensorRepository
{
    ...
    public function writeSensorData(array $data)
    {
        try {
            ...
            $point = Point::measurement('sensor_reading')
                ...
                ->addField('ni_estimated', (float) ($data['ni_estimated'] ?? 0))
            ...
            \Illuminate\Support\Facades\Log::warning("InfluxDB write failed (Offline/Connection error): " . $e->getMessage());
        }
    }
    ...
    public function getLatestReadings($limit = 10)
    {
        try {
            ...
            \Illuminate\Support\Facades\Log::warning("InfluxDB query failed (Offline/Connection error): " . $e->getMessage());
            return [];
        }
    }
        ...
        try {
            ...
            foreach ($records as $table) {
                foreach ($table->records as $r) {
                    ...
                    $obj->ni_estimated = isset($r['ni_estimated']) && $r['ni_estimated'] !== null ? (float) $r['ni_estimated'] : 0;
    ...
    public function getDailyStats()
    {
        try {
            ...
            return [
                ...
                'avg_ni' => $countNi > 0 ? round($sumNi / $countNi, 5) : 0,
            ];
        } catch (\Exception $e) {
            \Illuminate\Support\Facades\Log::warning("InfluxDB daily stats failed (Offline/Connection error): " . $e->getMessage());
            return [
                'total' => 0,
                'warning' => 0,
                'danger' => 0,
